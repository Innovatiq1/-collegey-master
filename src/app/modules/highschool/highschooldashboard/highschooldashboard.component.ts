...
@Component({
  selector: 'app-highschooldashboard',
  templateUrl: './highschooldashboard.component.html',
  styleUrls: ['./highschooldashboard.component.css']
})
export class HighschooldashboardComponent implements OnInit,OnDestroy {
 modalRef: BsModalRef;  
...
invitestudent(template: TemplateRef<any>){
  this.modalRef = this.modalService.show(template);
  this.modalRef.setClass("modal-width");
}
invitealumni(template: TemplateRef<any>){
  this.modalRef = this.modalService.show(template);
  this.modalRef.setClass("modal-width");
}
collegeyopp(check){
